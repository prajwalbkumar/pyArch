...
try:
                ...
                if hosted_wall_rating:
                    if hosted_wall_rating.HasValue:
                        if hosted_wall_rating.AsString() == "" or hosted_wall_rating.AsString() == "0":
                            ...
                            count = count + 1
                            if "-" in hosted_wall_rating.AsString():
                                continue
                            elif door_rating_param.AsString() == str(int(int(hosted_wall_rating.AsString()) - 20)):
                                continue
                            else:
    ...
    if skipped_doors:
        report = forms.alert("Door STC ratings require attentions", title="Door STC Rating Missing", warn_icon=True, options=["Show Report"]) 
        failed_data = []
        ...
        for door in skipped_doors:
                ...
                failed_data.append([output.linkify(door.Id), door_mark, door_level, door_room_name, door_room_number, hosted_wall_rating.AsString(), door.LookupParameter("STC_Rating").AsString(), "INVALID HOST RATING"])
        ...
        if failed_doors:
            ...
            for door in failed_doors:
                    ...
                    failed_data.append([output.linkify(door.Id), door_mark, door_level, door_room_name, door_room_number, hosted_wall_rating.AsString(), door.LookupParameter("STC_Rating").AsString(), str(int(int(hosted_wall_rating.AsString()) - 20))])
